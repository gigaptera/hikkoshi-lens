...
const NAV: NavItem[] = [
  { href: "/", label: "ダッシュボード", icon: House, description: "進捗と次の作業" },
  { href: "/search", label: "勤務地を入力", icon: MapPinLine, description: "住所を設定" },
  { href: "/stations", label: "候補駅を見る", icon: MagnifyingGlass, description: "おすすめ駅ランキング" },
  { href: "/lines", label: "路線で見る", icon: Train, description: "路線別に比較" },
];
        ...
        <div className="p-6 pt-0 space-y-4 text-sm text-neutral-700">
          <div className="space-y-1">
            ...
            <div className="text-neutral-600">
              最寄り駅名でもOKです（例: 大手町 / 渋谷）。住所が曖昧でも検索できます。
            </div>
          </div>
          <div className="space-y-1">
      ...
      <div className="px-2">
        ...
        <nav className="flex flex-col gap-1">
          {NAV.map((item) => {
            ...
            const disabled = item.href !== "/search" && !workLocation && item.href !== "/";
            return (
              <Link
                key={item.href}
              ...
              >
                <div
                  className={cn(
                    ...
                    active ? "bg-white border-primary-200" : "bg-white border-border"
                  )}
                >
                  <Icon weight={active ? "fill" : "light"} className="text-neutral-800" />
                </div>
